...
class Evaluator:
    ...
    def _already_evaluated(self, message_id: int) -> bool:
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT 1 FROM evaluation_results WHERE message_id = %s LIMIT 1",
                    (message_id,),
                )
    ...
    def _get_or_create_run(self) -> int:
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT id FROM evaluation_runs WHERE judge_model = %s ORDER BY id DESC LIMIT 1",
                    (JUDGE_MODEL,),
                )
                row = cur.fetchone()
                if row:
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    JOIN messages m ON m.id = er.message_id
