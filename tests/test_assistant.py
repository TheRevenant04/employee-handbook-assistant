...
class TestEnvBool:
    def test_env_bool_true_values(self):
        from app.services.rag_service import env_bool
...
class TestCreateAssistant:
    ...
    @patch("app.services.rag_service.get_query_rewriter")
...
class TestGetReranker:
    ...
    @patch("app.services.rag_service.Reranker", side_effect=Exception("load error"))
    @patch("app.services.rag_service.os.path.exists", return_value=True)
    @patch.dict(os.environ, {"RERANKER_ENABLED": "true"})
...
class TestGetQueryRewriter:
    ...
    def test_returns_none_when_disabled(self, mock_rewriter_cls):
        from app.services.rag_service import get_query_rewriter
